...
export default ({ caseId, userMap, setExecuteStatus, project }) => {
  ...
  const CaseTitle = (
    <div>
      <span>用例详情</span>
      <Button type="danger" className={styles.inlineButton}>
        <DeleteOutlined />
        删除
      </Button>
  ...
  const onFinish = async () => {
    ...
    const params = {
      ...
      id: caseId,
      request_header: translateHeaders(),
      body,
    };
          ...
          {!editing ? (
            <Descriptions title={CaseTitle} bordered size="middle" span={3}>
              ...
              <Descriptions.Item label="请求url" span={3}>
                <a href={data.url} style={{ fontSize: 16 }}>
                  {data.url}
                </a>
              </Descriptions.Item>
